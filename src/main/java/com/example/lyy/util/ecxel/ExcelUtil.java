package com.example.lyy.util.ecxel;
	...
	public boolean exportExcel(List<T> list, String sheetName, int sheetSize,
		...
		for (int index = 0; index <= sheetNo; index++) {
			...
			for (int i = 0; i < fields.size(); i++) {
                ...
                cell.setCellValue(annotation.value());// 写入列名
				...
				for (int j = 0; j < fields.size(); j++) {
					...
					try {
						...
						if (attr.isExport()) {
                            ...
                            String s = String.valueOf(field.get(vo));
                            cell.setCellType(HSSFCell.CELL_TYPE_STRING);
                            if (attr.map().length == 0){
                                cell.setCellValue(field.get(vo) == null ? ""
                                        : new String(s.getBytes(), StandardCharsets.UTF_8));// 如果数据存在就填入,不存在填入空格.
                            }else {
                                String[] map = attr.map();
                                for (int k = 0; k < map.length; k++) {
                                    String str = map[i];
                                    String[] split = str.split(":");
                                    if (split[0].equals(field.get(vo))) {
                                        cell.setCellValue(field.get(vo) == null ? ""
                                                : new String(split[1].getBytes(), StandardCharsets.UTF_8));
                                    }
                                }
                            }
						}
